...
foldseek_3di = pd.read_csv('/cbscratch/rad/e_bench_archive/foldseek/fs_results/fs_calc_full', sep='\t')
foldseek_lol = pd.read_csv('/cbscratch/rad/e_bench_archive/lol_aln/results/lol_calc_full_md_f', sep='\t')
# LoL-align (cs3) alignments re-ranked per query by the v20 homology-classifier probability.
foldseek_lol_v20 = pd.read_csv('/cbscratch/lolalignBenchmark/lol_calc_full_md_cs3_v20pred.tsv', sep='\t', low_memory=False)
dali_aln = pd.read_csv('/cbscratch/rad/e_bench_archive/dali/final_results/dali_calc_full', sep='\t')
tm_aln_qtm = pd.read_csv('/cbscratch/rad/e_bench_archive/tm_aln/final_results/tm_aln_qtm_final', sep='\t')
softalign_aln = pd.read_csv('/cbscratch/lolalignBenchmark/softalign_score_calc_full.tsv', sep='\t')

print(f'Foldseek: {len(foldseek_3di)} alignments')
print(f'LoL-align (cs3): {len(foldseek_lol)} alignments')
...
alignments = [foldseek_3di, foldseek_lol, dali_aln, tm_aln_qtm, softalign_aln]
...
# SoftAlign native ranking = softmax score (per-query CSV files).
import glob as _glob
_hp = '/cbscratch/lolalignBenchmark/e_bench/softalign/softmax_scores_highplddt/output'
_rows = []
for _f in _glob.glob(os.path.join(_hp, 'scores_sorted_*.csv')):
    _q = os.path.basename(_f)[len('scores_sorted_'):-len('.csv')]
    _d = pd.read_csv(_f); _d.columns = ['target', 'softmax_score']; _d['query'] = _q
    _rows.append(_d[['query', 'target', 'softmax_score']])
_softmax = pd.concat(_rows, ignore_index=True)
...
alignments[4] = softalign_aln

# LoL-align (v20): same cs3 alignments, re-ranked per query by predicted homology probability.
...
alignment_names = ['foldseek_3di', 'foldseek_lol', 'dali_aln', 'tm_aln_qtm', 'softalign_aln', 'foldseek_lol_v20']
display_names = {
    'foldseek_3di': 'Foldseek',
    'foldseek_lol': 'LoL-align',
    'dali_aln': 'Dali',
    'tm_aln_qtm': 'TM-align',
...
SAVE_DIR = '/cbscratch/lolalignBenchmark/plots_full'
os.makedirs(SAVE_DIR, exist_ok=True)


def _plot_order(cols):
    """Draw LoL-align probability last so it renders on top of the rs-seed cluster."""
    return [c for c in cols if c != 'LoL-align probability'] + \
           [c for c in cols if c == 'LoL-align probability']
